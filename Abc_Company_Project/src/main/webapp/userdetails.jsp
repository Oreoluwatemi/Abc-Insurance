...
<br/><br/><br/>
<div class="container">  
<h1>Registered Devices</h1><br/>
<%
	ArrayList<RegisteredDevice> list = new ArrayList<RegisteredDevice>();
    list = (ArrayList<RegisteredDevice>) request.getAttribute("devicelist");
    
    ArrayList<Claim> clist = new ArrayList<Claim>();
    clist = (ArrayList<Claim>) request.getAttribute("claimlist");
 ...
 <table class="table" >
        ...
        <tbody>
            <% for(int i = 0; i < list.size(); i++) {
                ...
                device = list.get(i);
                if(user.equals(device.getUsername())){
            %>
            
            <tr>
                <td><%=device.getUsername()%></td>
                <td><%=device.getProductname()%></td>
                <td><%=device.getSerialno()%></td>
...
<table class="table">
        ...
        <tbody>
            <% for(int i = 0; i < clist.size(); i++) {
                ...
                claim = clist.get(i);
                if(user.equals(claim.getUsername())){
            %>
            <tr>
